...
export function LeadsTable() {
  ...
  const [selectedLeadForNotes, setSelectedLeadForNotes] = useState<string | null>(null);

  const selectedLead = leads.find(lead => lead.id === selectedLeadForNotes);
              ...
              {leads.map((lead) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                    {lead.notes ? (
                      <button
                        onClick={() => setSelectedLeadForNotes(lead.id)}
                        className="inline-flex items-center gap-1.5 text-slate-400 hover:text-indigo-500 transition-colors p-1 rounded hover:bg-slate-100"
                        title="View notes"
                      >
